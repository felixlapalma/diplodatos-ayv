...
def plot_components_basic_decomp(results, label=''):
    colours=['#D62728', '#FF7F0E', '#2CA02C', '#1F77B4']
    fig, ax = plt.subplots(2,2, figsize=(12,8))
    ax[0,0].plot(results.observed, color=colours[0], alpha=0.95)
    ax[0,0].set(ylabel=label, title='Observed')
    ...
    ax[1,1].set(title='Irregular')
    sns.despine()
    fig.suptitle('Time series decomposition  (basic decomposition)', fontsize=13.5)   
    fig.tight_layout()
    fig.subplots_adjust(top=0.9)
    return fig, ax


def plot_components_x13_mod(results_ini, label=''):
    colours=['#D62728', '#FF7F0E', '#2CA02C', '#1F77B4']
    fig, ax = plt.subplots(2,2, figsize=(12,8))
    cases=['observed','trend','irregular','seasadj']
    results_changed_flag=False
    from copy import deepcopy
    results=deepcopy(results_ini)
    for c in cases:
        if type(rgetattr(results,c+'.index'))==pd.core.indexes.period.PeriodIndex:
            rsetattr(results,c+'.index',rgetattr(results,c+'.index').to_timestamp())
            results_changed_flag=True
    ax[0,0].plot(results.observed, color=colours[0], alpha=0.95)
    ax[0,0].set(ylabel=label, title='Observed')
    ax[0,1].plot(results.trend, color=colours[1], alpha=0.95)
    ax[0,1].set(title='Trend')
    ...
    fig.tight_layout()
    fig.subplots_adjust(top=0.9)
    if results_changed_flag:
        print('Datetime was generated for plotting based on PeriodIndex')
    return fig, ax, results_ini
